...
from tkinter import ttk  # Added for Progressbar
import subprocess
import time
import threading
...
class PyvoDashboardApp:
    ...
    def run_script(self):
        ...
        # Run script in a separate thread to avoid blocking the UI
        threading.Thread(target=self.execute_script, args=(script_path, selected_script), daemon=True).start()

    def execute_script(self, script_path, selected_script):
    ...
    def update_settings(self):
        ...
        settings = {
            "Monitoring": self.var_monitoring.get(),
            "Logging": self.var_logging.get(),
            "Performance": self.var_performance.get(),
            "Error Handling": self.var_error_handling.get(),
        }
        
        for key, value in settings.items():
            setattr(pyvo_integration, f"enable_{key.lower()}", value)
